...
from sae_clip import Dotdict

import os
import sys
sys.path.append(os.path.dirname(os.path.abspath(__file__)))
from src.my_utils import my_clip_forward_factory
...
class SAECLIP(nn.Module):
    ...
    def __init__(self, clip_model: ClipLike, num_debias_tokens: int, hidden_dim: int = 512, max_tokens: int = 77, layer: int = 23, layer_txt: int = 11,
        ...
        import inspect
        print('Where is this file (self.clip.encode_image) located at?', inspect.getfile(self.clip.encode_image))
        print('Where is this file (self.clip.encode_text) located at?', inspect.getfile(self.clip.encode_text))
        ...
        try:
            self.base_CLIPEncoderLayerPostMlpResidual_txt = copy.deepcopy(
                self.clip.transformer.resblocks[self.layer_txt]
            )
        except:
            print('SAE (text encoder) not properly initialized...')
            pass
    ...
    def encode_text(self, text, is_proj=True):
    ...
    def encode_image(self, x, is_proj=True):
        return self.clip.encode_image(x, is_proj)
    
    def attach_and_fix(self, sae, neurons_to_fix={}, pre_zero=False, is_image=True, alpha=0.6):
        modified_sae = SAEWrapper(sae, neurons_to_fix, pre_zero)
        if is_image:
            self.clip.visual.transformer.resblocks[self.layer] = CLIPEncoderLayerPostMlpResidual_notfromLLaVA(
                self.base_CLIPEncoderLayerPostMlpResidual,
                modified_sae,
                alpha=alpha
            )
        else:
            self.clip.transformer.resblocks[self.layer_txt] = CLIPEncoderLayerPostMlpResidual_notfromLLaVA(
                self.base_CLIPEncoderLayerPostMlpResidual_txt,
                modified_sae,
                alpha=alpha
            )
    ...
    def attach_and_fix_saeembs(self, sae, neurons_to_fix={}, pre_zero=False, is_image=True):
        ...
        if is_image:
            ...
            self.clip.transformer.resblocks[self.layer_txt] = CLIPEncoderLayerPostMlpResidual_notfromLLaVA_saeembs(
                self.base_CLIPEncoderLayerPostMlpResidual_txt,
                modified_sae,
            )

    def save_neurons_clip(
        model,
        prompt, 
        sae,
        neuron,
        ...
        emb = model.prompt(prompt, image, max_tokens=30)
