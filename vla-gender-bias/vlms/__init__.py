...
from utils_new.configs import model_to_variant_mapping
...
def prune_model(model: BaseVLM, model_name: str, sparsity: float) -> BaseVLM:
    ...
    print(model, 'which model', model_type)
    apply_pruning(
        model=model,
        sparsity=sparsity,
        granularity="local",
...
def load_model(name: str, alpha=0.6) -> BaseVLM:
    print(name)
    if name.startswith("sae:"):
        ...
        if 'llavavision' in name:
            from models.llavaonevision import LlavaOneVision
            model = LlavaOneVision(is_vision=False)

            if name.endswith(":attach"):
                expansion_factor = 8
                data_type = 'fairface'
                clip_type = 'llava-onevision-qwen2-7b-ov-hf'
                layer = 25
                epoch = 100000
            ...
            if name.endswith(":attach"):
                ...
                sae_path = f"/workspace/cvml_user/namin/bias_vlm/sae-for-vlm/checkpoints_dir/matroyshka_batch_top_k_20_x{expansion_factor}/random_k_2/{data_type}_train_activations_{clip_type}_{layer}_post_mlp_residual_matroyshka_batch_top_k_20_x{expansion_factor}/trainer_0/checkpoints/ae_{epoch}.pt"
                sae = MatroyshkaBatchTopKSAE.from_pretrained(sae_path).cuda()
                # neurons_to_fix = {386:0, 196:0} # gender neuron
    ...
    if name.startswith("tuned:"):
        ...
        if tuning_mode == "full":
            state_dict = torch.load(
                "/workspace/cvml_user/namin/bias_vlm/vla-gender-bias/model.pt", #os.path.join(path_to_tuned_model, "full", "model.pt"),
                map_location="cpu",
            )
            ...
            llm_layers_lora = PeftModel.from_pretrained(
                llm_layers,
                "/workspace/cvml_user/namin/bias_vlm/vla-gender-bias/lora" #os.path.join("path_to_tuned_model", "lora"),
            )
    ...
    if name.startswith("prompt-tuned"):
        _, name = name.split(":")
        model = load_model(name)
        prefix = torch.load("/workspace/cvml_user/namin/bias_vlm/vla-gender-bias/tuning_prefix.pt") #os.path.join("./results/prompt_tuning", name, "tuning_prefix.pt"))
