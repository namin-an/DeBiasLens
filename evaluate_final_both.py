...
def main(args):
    ...
    if args.md_flag == 'clip':
        ...
        clip_type = 'vit-large-patch14-336' #'vit-base-patch16' #'vit-large-patch14-336'
        md_flag = "ViT-L/14@336px" # "ViT-B/16" #"ViT-L/14@336px"
        is_image = True
        is_text = True
        neurons_to_fix_txt = {1200:0, 1057:0} #{} #{1200:0, 1057:0}
        neurons_to_fix_img = {458:0, 375:0} #{6:0, 264:0, 279:0, 98:0, 454:0, 364:0} #{105:0, 163:0, 102:0} #{98:0, 195:0} #{458:0, 375:0} 
    ...
    with warnings.catch_warnings():
        ...
        if sae_txt == None or sae_img == None:
            ...
            deb_clip_model, preprocess = sae_clip.load(md_flag, device=device, layer=layer_img, layer_txt=layer_txt)            
            try:
                deb_clip_model.eval()
            except:
            ...
            for attribute in attribute_lst:
                for prmpt in prompts:
                    for alpha in [0.6]: #, 1.0]: #[0.0, 0.2, 0.4, 0.5, 0.6, 0.8, 1.0]:
                        deb_clip_model.attach_and_fix(sae=sae_txt, neurons_to_fix=neurons_to_fix_txt, pre_zero=False, alpha=alpha, is_image=False)
                        deb_clip_model.attach_and_fix(sae=sae_img, neurons_to_fix=neurons_to_fix_img, pre_zero=False, alpha=alpha, is_image=True)
                        val, df = sae_clip.measure_bias(deb_clip_model, img_preproc=preprocess, tokenizer=tokenizer, attribute=attribute,
                                                    prompt=prmpt, mod=args.mod, md_flag=args.md_flag, data_type=eval_data_type,
                                                    is_siglip=is_siglip, is_blip=is_blip, epoch=epoch,
                                                    suppress_list=suppress_list, excite_dict=excite_dict,
                                                    sae=sae_img, sae_text =sae_txt)
                        print(alpha, val)
